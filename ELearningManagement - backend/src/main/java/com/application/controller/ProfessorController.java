...
public class ProfessorController
{
  ...
  private ChapterRepository chapterRepo;
	...
	@CrossOrigin(origins = "http://localhost:4200")
	public void addNewChapters(@RequestBody Chapter chapter) throws Exception
	{
		Chapter chapterObj = null;
    String x=chapter.getCoursename();
    System.out.println("-----------"+x+"-------------");
    Integer chap=1;
    if(chapterService.getChapter1(x)!=0)
    {

      chap=1;
    }
    else if(chapterService.getChapter2(x)!=0)
    {
      chap=2;
    }
    else if(chapterService.getChapter3(x)!=0)
    {
      chap=3;

    }
    if (chap!=1) {
      chapterService.addNewChapter(x, chapter.getChapter1name(), chapter.getChapter1id());
    }
    else
    {

      Chapter new1=new Chapter(chapter.getId(),chapter.getCoursename(),chapter.getChapter1name(),chapter.getChapter1id(),chapter.getChapter2name(),chapter.getChapter2id(),chapter.getChapter3name(),chapter.getChapter3id(),chapter.getChapter4name(),chapter.getChapter4id(),chapter.getChapter5name(),chapter.getChapter5id(),chapter.getChapter6name(),chapter.getChapter6id(),chapter.getChapter7name(),chapter.getChapter7id(),chapter.getChapter8name(),chapter.getChapter8id());
      chapterRepo.save(new1);
    }
